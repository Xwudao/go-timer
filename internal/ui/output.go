...
func Success(format string, args ...any) {
	_, _ = fmt.Fprint(os.Stdout, successColor.Sprint("✔ ")+fmt.Sprintf(format, args...)+"\n")
}
...
func Warn(format string, args ...any) {
	_, _ = fmt.Fprint(os.Stdout, warnColor.Sprint("⚠ ")+fmt.Sprintf(format, args...)+"\n")
}

// Info prints a cyan informational message.
func Info(format string, args ...any) {
	_, _ = fmt.Fprint(os.Stdout, infoColor.Sprint("ℹ ")+fmt.Sprintf(format, args...)+"\n")
}

// Header prints a bold white header.
func Header(text string) {
	_, _ = fmt.Fprintln(os.Stdout, headerColor.Sprint(text))
}

// Dim prints a dimmed/muted message.
func Dim(format string, args ...any) {
	_, _ = fmt.Fprint(os.Stdout, dimColor.Sprintf(format, args...)+"\n")
}

// Accent prints an accented (magenta) message.
func Accent(format string, args ...any) {
	_, _ = fmt.Fprint(os.Stdout, accentColor.Sprintf(format, args...)+"\n")
}

// Print is a plain print.
func Print(format string, args ...any) {
	_, _ = fmt.Fprint(os.Stdout, fmt.Sprintf(format, args...)+"\n")
}
...
func Table(headers []string, rows [][]string) {
	...
	_, _ = fmt.Fprintln(w, strings.Join(headerStrs, "\t"))
	...
	_, _ = fmt.Fprintln(w, dimColor.Sprint(strings.Join(seps, "\t")))
...
func Prompt(label, defaultVal string) string {
	if defaultVal != "" {
		_, _ = fmt.Fprint(os.Stdout, accentColor.Sprint("? ")+label+dimColor.Sprintf(" [%s]", defaultVal)+": ")
	} else {
		_, _ = fmt.Fprint(os.Stdout, accentColor.Sprint("? ")+label+": ")
	}
	var input string
...
func PromptRequired(label string) (string, error) {
	for {
		_, _ = fmt.Fprint(os.Stdout, accentColor.Sprint("? ")+label+warnColor.Sprint(" (required)")+": ")
		var input string
		_, _ = fmt.Scanln(&input)
...
func Confirm(label string, defaultYes bool) bool {
	...
	_, _ = fmt.Fprint(os.Stdout, accentColor.Sprint("? ")+label+" "+dimColor.Sprint(hint)+": ")
	var input string
	_, _ = fmt.Scanln(&input)
...
func DryRunNotice() {
	_, _ = warnColor.Println("⚡ Dry-run mode — no changes will be made")
}

// Separator prints a horizontal separator line.
func Separator() {
	_, _ = dimColor.Println(strings.Repeat("─", 60))
}
